...
//         camera.position.set(107,26,-59);
    ...
    workerLoadVsg.onmessage=function(event) {
        ...
        //根据vsg中的数据绘制cube来检测是否正常
        //输出key值
        // for(var key in vsgData)
        // {
        //     var index1 = key.indexOf('-');
        //     var vsgX = Math.floor((Number(key.substring(0,index1))*cashVoxelSize + cashSceneBBoxMinX)/500);
        //     var index2 = key.lastIndexOf('-');
        //     var vsgZ = Math.floor((Number(key.substring(index1+1,index2))*cashVoxelSize+ cashSceneBBoxMinY)/500);
        //     var vsgY = Math.floor((Number(key.substring(index2+1))*cashVoxelSize + cashSceneBBoxMinZ)/500);
        //
        //     if(vsgY<2 && vsgY>=1){
        //         console.log(vsgX+"  "+vsgY+"  "+vsgZ);
        //         var vsgCube = new THREE.Mesh(new THREE.CubeGeometry(1,1,1),new THREE.MeshBasicMaterial({color:0xff0000}));
        //         vsgCube.position.set(vsgX,vsgY,vsgZ);
        //         scene.add(vsgCube);
        //     }
        //
        // }
    ...
    function createRandomPos(meshNum) {
        var blendMeshPosIndexArr = ["74&11@0","77&82@0","24&84@0","83&-18@0"];
